...
for i in files:
    ...
    df["distance_leftkneeankle_anomalien"] = df["z_leftkneeankle"].apply(
        check_anomalien
    )
    df["distance_rightkneeankle_anomalien"] = df["z_rightkneeankle"].apply(
        check_anomalien
    )

    df["distance_leftkneeheel_anomalien"] = df["z_leftkneeheel"].apply(check_anomalien)
    df["distance_rightkneeheel_anomalien"] = df["z_rightkneeheel"].apply(
        check_anomalien
    )

    df["distance_leftankleindex_anomalien"] = df["z_leftankleindex"].apply(
        check_anomalien
    )
    df["distance_rightankleindex_anomalien"] = df["z_rightankleindex"].apply(
        check_anomalien
    )

    df["distance_leftheelindex_anomalien"] = df["z_leftheelindex"].apply(
        check_anomalien
    )
    df["distance_rightheelindex_anomalien"] = df["z_rightheelindex"].apply(
        check_anomalien
    )

    # Entfernen aller Columns mit Z-Werten, optional
    df_anomalien = df.drop(
        [
            "z_hips",
            "z_leftkneehip",
            "z_rightkneehip",
    ...
    df_anomalien.to_csv(
        "model_evaluation/evaluated_distances/" + "evaluated_" + x[2], index=False
    )
